...
const OverlayApp: React.FC<OverlayAppProps> = ({ onClose, onHeaderReady }) => {
  ...
  const [a11y, setA11y] = useState<A11yState>({ largeText: false, highContrast: false, reduceMotion: false });
  ...
  return (
    <div className={`min-h-full h-full w-full bg-white text-slate-700 ${a11y.largeText ? 'lb-large-text' : ''} ${a11y.highContrast ? 'lb-high-contrast' : ''} ${a11y.reduceMotion ? 'lb-reduce-motion' : ''}`}>
      <div ref={headerRef} id="lb-header" className="flex items-center justify-between px-4 py-3 bg-slate-50 border-b border-slate-200 select-none">
        ...
        <button title="Close" className="rounded-lg px-3 py-2 text-slate-600 hover:bg-slate-100" onClick={onClose}>×</button>
      </div>

      <div className="h-[calc(100%-56px)] overflow-auto p-5 bg-white">
